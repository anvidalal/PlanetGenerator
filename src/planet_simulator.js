...
function Space() {
    ...
    space.initializePlanets = function (scene, camera, renderer, gui) {
        space.bodies[0] = sun;
        space.bodies[1] = earth;
        numPlanets = 2;

        for (var i = 1; i < space.bodies.length; i++) { //change back to 1
            space.bodies[i].addPlanet(scene);

            // add controls for earth
            space.bodies[i].addControls(scene, camera, renderer, gui);
            //console.log("controls created");
        }
    }

    space.addNewPlanet = function (scene, camera, renderer, gui) {
        var newPlanet = new Planet.Planet(5.974e24, 6.371e3, new THREE.Vector3(1.496e11, 0.000e00, 0.000e00), new THREE.Vector3(0.000e00, 1.490e04, 0.000e00), space.scale, numPlanets);
        space.bodies[numPlanets] = newPlanet;

        space.bodies[numPlanets].addPlanet(scene);

        //controls
        space.bodies[numPlanets].addControls(scene, camera, renderer, gui);
    ...
    space.simulate = function (orbiting, camera) {
        ...
        for (var i = 0; i < numPlanets; i++) {
            space.bodies[i].incrementPosition(space.timeStep, orbiting);
        }
